...
const LoginPage: React.FC = () => {
  ...
  useEffect(() => {
    setMounted(true);
    document.body.classList.add('bg-slate-950'); //Added dark mode styling
    document.documentElement.classList.add('dark'); //Added dark mode styling
    return () => {
      setMounted(false);
  ...
  return (
    <div className="login-container min-h-screen w-full flex items-center justify-center bg-gradient-to-br from-blue-950 via-[#0c1525] to-gray-950 text-white"> {/* Kept original background */}
      <div className="absolute inset-0 overflow-hidden"> {/*Kept original background circles */}
      ...
      <div className="login-page-container relative z-10 w-full max-w-6xl flex flex-col md:flex-row mx-auto p-4 gap-12"> {/* Modified to use max-w-6xl and gap-12 for better layout */}
        <div className="login-hero md:w-1/2 flex flex-col justify-center space-y-6 p-6 md:p-12"> {/* Kept original hero section */}
          ...
          <h2 className="text-4xl md:text-5xl font-bold leading-tight text-white"> {/* Added text-white for better visibility */}
            {mounted && (
        ...
        <div className="login-form-container md:w-1/2 flex items-center justify-center py-8">
          <Card className="login-card w-full max-w-md space-y-6 bg-[#0c1a2e]/90 backdrop-blur-lg p-8 rounded-2xl shadow-2xl border border-blue-900/30 text-white transform transition-all duration-300 hover:shadow-blue-900/20 hover:border-blue-800/40"> {/* Kept original card styling */}
